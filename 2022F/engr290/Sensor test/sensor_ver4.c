...
int main(){
	...
	int pulseDist;							// distance of the pulse
	int adcValue;
	...
	while (1){
		...
		pulseDist = 43;							// quick maths w/ speed of sound to find real distance, '43' is just randomly chosen
		itoa(pulseDist, pulseDistStr, 10);    		// convert int to string (base of 10)
		
		pointerString("Distance: "); 				// print  
		pointerString(pulseDistStr);				// print value of the distance in cm
		pointerString(" cm \n");					// print 
		
		yellowLed(pulseDist);				// PB5 yellow led out of range detector
		
		adcValue=adcConvertion();
		distance= calculateDistance(adcValue);
		pointerString("Distance in cm : ");
		pointerString(distance);
	}
	return 0;
}
